 ...
 * @param {SchemaConflictHandler} [opts.onSchemaConflict] Called when a remote (CRDT-merged) change could not be represented against the ProseMirror schema and the binding had to reshape, drop, or invent content to recover — the schema-invalid-merge case documented in CAVEATS.md ("Schema mismatches under concurrency", upstream #258). Receives `{ change, fix, wholesaleReplace }`: `change` is the incoming delta, `fix` the delta describing exactly what the binding altered relative to the merged state (this fix is also written back to the Y document), and `wholesaleReplace` whether recovery required replacing the whole document through ProseMirror's fitting algorithm. Use it to warn users that concurrent edits collapsed content instead of losing it silently. Exceptions from the handler are caught and logged.
 * @returns {Plugin}
 */
    ...
    view () {
      ...
      const setup = (view, pluginState) => {
        ...
        const pmRdt = new ProsemirrorRdt({
          ...
          compare,
          onSchemaConflict: opts.onSchemaConflict ?? null,
